...
function checkLoadFromUrl() {
    ...
    if (loadId) {
        ...
        fetch(gasUrl)
            .then(res => res.json())
            .then(data => {
                const charData = data.find(item => item.id === loadId);
                if (charData && charData.state) {
                    state.selections = charData.state.selections || {};
                    state.locks = charData.state.locks || {};
                    state.lastBossResult = charData.state.lastBossResult || null;
                    state.customOriginFactor = charData.state.customOriginFactor !== undefined ? charData.state.customOriginFactor : null;
                    state.characterImage = charData.state.characterImage || null;
                    state.freeText = charData.state.freeText || {};
                    
                    window.editingCharId = charData.id;
                    window.editingCharName = charData.name;
...
function publishData() {
    ...
    if (!state.selections.limitLevel || !state.selections.origin || !state.selections.ability || 
        !state.selections.item || !state.selections.talent || !state.selections.connection) {
        alert("公開するには、「限界レベル」「出身」「能力値」「特殊アイテム」「タレント」「コネクション」のすべてを設定してください。");
        return;
    }

    let action = "publish";
    let charId = "char_" + Date.now() + "_" + Math.floor(Math.random() * 1000);
    let charName = "";

    if (window.editingCharId) {
        const overwrite = confirm(`ギャラリーから読み込んだ「${window.editingCharName}」を編集しています。\nこのキャラクターのデータを上書き更新しますか？\n（「キャンセル」を押すと新しい別のキャラクターとして公開します）`);
        if (overwrite) {
            action = "update";
            charId = window.editingCharId;
            charName = window.editingCharName;
        } else {
            window.editingCharId = null; 
            window.editingCharName = null;
        }
    }

    if (action === "publish") {
        charName = prompt("公開するキャラクターの名前を入力してください（空欄でも可）", "名無し");
        if (charName === null) return;
    }

    const password = prompt(action === "update" ? "更新用のパスワードを入力してください（未設定の場合はそのままOK）:" : "後から編集・削除するためのパスワードを設定してください（任意・空欄でも可）:");
    ...
    elements.publishBtn.textContent = action === "update" ? "更新中..." : "公開中...";
    ...
    .then(result => {
        ...
        if (result.result === "success") {
            alert(action === "update" ? "上書き更新が完了しました！" : "ギャラリーに公開しました！");
            window.editingCharId = charId;
            window.editingCharName = charName;
        } else {
            alert("エラー: " + (result.message || "パスワードが間違っている可能性があります。"));
        }
    })
    .catch(error => {
        console.error(error);

document.addEventListener('DOMContentLoaded', () => {
    ...
    function createCard(item) {
        ...
        levelRow.innerHTML = `<span class="gallery-stat-label">推定最終Lv</span><span class="gallery-stat-value highlight">${finalLevel}</span>`;
        stats.appendChild(levelRow);

        ['limitLevel', 'origin', 'ability'].forEach(key => {
            const row = document.createElement('div');
            row.className = 'gallery-stat-row';
            const val = item.state.selections[key] ? `ランク ${item.state.selections[key]}` : '未設定';
            row.innerHTML = `<span class="gallery-stat-label">${rankLabels[key]}</span><span class="gallery-stat-value">${val}</span>`;
